...
export class TaskManagementDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    { label: 'Task Management', path: '/task/task-management' },
  ...
  openCommentModal(item: any) {
    const modalRef = this.modalService.open(TaskCommentModalComponent, { centered: true, size: 'xl', scrollable: true, backdrop: 'static' });
    if (item) {
      modalRef.componentInstance.item = item;
    }
    modalRef.componentInstance.title = item ? 'Edit' : 'Create';
